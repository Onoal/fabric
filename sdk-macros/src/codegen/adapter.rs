...
pub fn expand_adapter(input: &AdapterInput) -> TokenStream {
    if input.target_kind.is_none() {
        return expand_canonical_adapter(input);
    }
    let sdk = fabric_path();
    ...
    let interface = input
        ...
        .as_ref()
        .expect("legacy adapter! headers declare an implementation interface");
    ...
    let target_kind = input
        .target_kind
        .expect("legacy adapter! headers declare a target kind");
    let schema_support_ty = schema_support_type(&sdk, target_kind);
    let schema_support_expr = schema_support_expr(&sdk, target_kind, target, input.schema.as_ref());
