...
use fabric_core::{
    CompositionError, ContractCompatibilityRequirement, ContractId, ContractIdentity, ContractKey,
    ContractRequirement, ContractVersion, ContractVersionRequirement, ModuleBindings,
    ModuleContract, ModuleError, ModuleId, ModuleRuntime, ResolvedContract,
};
...
pub type OptionalNotesCapture = Arc<Mutex<Option<Option<String>>>>;
...
pub fn notes_contract_id() -> ContractId {
    ContractId::new("fabric.test.raw.notes").expect("static notes contract id")
}

pub fn notes_contract_key(version: &str) -> ContractKey<NotesContract> {
    ContractKey::versioned(
        notes_contract_id(),
        ContractVersion::parse(version).expect("static notes version"),
    )
}

pub fn notes_requirement(version: &str) -> ContractRequirement<NotesContract> {
    ContractRequirement::versioned(
        notes_contract_id(),
        ContractVersionRequirement::parse(version).expect("static notes requirement"),
    )
}

pub fn captured_resolution(capture: &NotesCapture) -> CapturedResolution {
    capture
        .lock()
        .expect("capture lock")
        .clone()
...
pub fn captured_optional_resolution(capture: &OptionalNotesCapture) -> Option<String> {
    capture
        .lock()
        .expect("capture lock")
        .clone()
        .expect("captured optional resolution")
}
...
#[derive(Clone)]
pub struct OptionalNotesConsumer {
    module_id: ModuleId,
    requirement: ContractRequirement<NotesContract>,
    capture: OptionalNotesCapture,
}

impl OptionalNotesConsumer {
    pub fn new(
        module_id: &str,
        requirement: ContractRequirement<NotesContract>,
        capture: OptionalNotesCapture,
...
impl ModuleRuntime for OptionalNotesConsumer {
    fn id(&self) -> &ModuleId {
        &self.module_id
    }
    ...
    fn bind(&mut self, bindings: &ModuleBindings) -> Result<(), ModuleError> {
        let resolved = bindings
            ...
            .map_err(|error| ModuleError::new(error.to_string()))?
            .map(|resolved| resolved.read());
        *self.capture.lock().expect("capture lock") = Some(resolved);
        Ok(())
    }
...
impl ModuleRuntime for MismatchedNotesConsumer {
    ...
    fn bind(&mut self, bindings: &ModuleBindings) -> Result<(), ModuleError> {
        ...
        *self.capture.lock().expect("capture lock") = Some(CapturedRequirementMismatch {
        ...
        Err(ModuleError::new("raw api requirement declaration mismatch"))
    }
